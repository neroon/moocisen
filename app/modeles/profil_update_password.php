...
if($verifSession==1){
	if($verif==1){
		...
		if($verifId==0){
			...
			header ("location: ../../vues/profil?erreur=Erreur mdp");
		}else if($verifId==1){
			echo "->mdp ne corresponde pas";
			header ("location: ../../vues/profil?erreur=mdp non identique");
		}else if($verif==1){
			updateIdResetPwd();
			header ("location: ../../vues/profil?ok=success");
		}
	}
	else{
		echo '<br>wrong form';
		header ("location: ../../vues/profil?erreur=Erreur formulaire");
	}
}else{
